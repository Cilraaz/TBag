...
TINVOPT_FRAME_HEIGHT = (TINVOPT_FRAME_LINE_HEIGHT*(TINV_OPTS_SCROLL_LINES+1)) + (TINVOPT_FRAME_BORDER*2) + TINVOPT_FRAME_BOTTOMPADDING
...
function TInvOpt_CreateCfgOpt()
  ...
  TBag:CreateCfgOpt(TInv_CfgOpt, TInvFrame.cfg, TInvFrame.bags, function () TInvFrame:UpdateWindow() end, TInvOpt_ResizeUpdate, TInvOpt_ForceUpdate)

  TBag:MakeCheck(TInv_CfgOpt, L["Alt Key Auto-Pickup:"], TInvFrame.cfg, "alt_pickup", TInvOpt_ResizeUpdate)
  TBag:MakeCheck(TInv_CfgOpt, L["Alt Key Auto-Panel:"], TInvFrame.cfg, "alt_panel", TInvOpt_ResizeUpdate)
...
function TInv_Options_InitWindow()
  ...
  TInv_Config_MaxScroll = mathmax( 1, (tgetn(TInv_CfgOpt)-TINV_OPTS_SCROLL_LINES)+2 )
  ...
  --Print(" config options size: "..tgetn(TInv_CfgOpt) )
  ...
  TInv_OptsFrame:SetBackdropBorderColor(TBag:GetColor(TInvFrame.cfg, "brdr_"..TBag.MAIN_BAR))
...
function TInv_Options_UpdateWindow()
  ...
  local current_opt = mathfloor(TInv_Opts_CurrentPosition)
...
function TInvOpts_AddCat()
  -- Add a blank entry
  tinsert(TInvFrame.cfg["item_search_list"], {L["UNKNOWN"], "", "", "", "", ""})
