...
if TBag.VERSION  == "\64project-version\64" then
  ...
  local dev = '-Alpha'
  local short_date = strgsub(strsub(date, strfind(date, '%d+-%d+-%d+')),'-','')
  TBag.VERSION = short_date..'-r'..strsub(rev, strfind(rev, '%d+'))..dev..'-Shefki'
end
...
function TBag:ClearNewItm(itmcache, bagarr)
  local bag
...
function TBag:GetItemInfo(itemid)
  if itemid then
    if tostring(itemid):sub(1,10) ~= "battlepet:" then
      local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, _, _, itemStackCount, itemEquipLoc, iconFileDataID, _, itemType, itemSubType = GetItemInfo(itemid)
      return itemName, tostring(itemType), tostring(itemSubType), itemRarity, itemLink, itemStackCount, itemEquipLoc, itemType
    else
...
function TBag:GetItemID(itemlink)
  if itemlink and type(itemlink) == "string" then
    local a,b,c,d,e,f,g,h,i,j = itemlink:match("item:(%d+):(%d*):(%d*):(%d*):(%d*):(%d*):(%-?%d*):(%-?%d*):?(%d*):?([^|]*)")
    if a then
      local itemstring = strjoin(":","item",a,b,c,d,e,f,g,h)
      return a, itemstring, j
    end
    a,b,c,d,e,f,g = itemlink:match("battlepet:(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(%d+)")
    if a then
      local itemstring = strjoin(":","battlepet",a,b,c,d,e,f,g)
      return -1, itemstring, nil, a
    end

    if strfind(itemlink, "keystone") then
      return 180653, itemlink
    end
  end
...
function TBag:PlacePrep(playername,place)
  if (place == "body") then
    return strformat(" on %s's %s",playername,place)
  elseif (place == "container") then
    return strformat(" as %s's %s",playername,place)
  else
    return strformat(" in %s's %s",playername,place)
  end
end

function TBag:AddSearchResult(itm, playername, place, playerid)
  -- Strip the unique id
  local itemstring = strgsub(itm[self.I_ITEMLINK], "(item:%d+:%d+:%d+:%d+:%d+:%d+:%-?%d+):%-?%d+","%1:0",1)
  ...
  if (itemlink) then
    self:PrintDEBUG("TBag:AddSearchResult "..count.." "..itemlink..self:PlacePrep(playername,place))
      ...
      for _, slotarr in pairs(bagarr) do
        for _, itm in pairs(slotarr) do
          ...
          if (itm[self.I_ITEMLINK]) and (itm[self.I_NAME]) then
            ...
            if (strfind(strlower(itm[self.I_NAME]), self.SrchText)) then
...
function TBag:DoSearch(srch)
  ...
  if (srch) then
    ...
    for itemlink, aResult in pairs(SrchResults) do
      if (not found) then
        DEFAULT_CHAT_FRAME:AddMessage(self.SCP..strformat(L["Search results for %q:"],srch), 1, 1, 1)
      end
    ...
    if (not found) then
      DEFAULT_CHAT_FRAME:AddMessage(self.SCP..SC_NONE..strformat(L["No results|r for %q"],srch))
    end
...
-- Eighth default line (top) - Empty and Act Ons
  ...
  for _, bag in ipairs(bagarr) do
    self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS",self:GetBagPosName(bag)), 29, reset)
  end

-- Seventh default line - Quests and Factions
  self:SetCatBar(cfg, "QUEST", 28, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","OTHER"), 28, reset)
...
-- Fourth default line - To Use or Sell
  self:SetCatBar(cfg, strformat("IN_%s_BAG","ENCH"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","ENG"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","GEM"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","HERB"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","MINE"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","LTHR"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","INSC"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","TACKLE"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","REAG"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","FRIDGE"), 16, reset)
  self:SetCatBar(cfg, strformat("IN_%s_BAG","UNKNOWN"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","ENCH"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","ENG"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","GEM"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","HERB"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","MINE"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","LTHR"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","INSC"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","TACKLE"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","REAG"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","FRIDGE"), 16, reset)
  self:SetCatBar(cfg, strformat("EMPTY_%s_SLOTS","UNKNOWN"), 16, reset)

  self:SetCatBar(cfg, "COOKING", 15, reset)

  self:SetCatBar(cfg, strformat("SOULBOUND_%s","RING"), 14, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","TRINKET"), 14, reset)

  self:SetCatBar(cfg, strformat("SOULBOUND_%s","01_HEAD"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","02_NECK"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","03_SHOULDER"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","04_BACK"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","05_CHEST"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","06_SHIRT"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","07_TABARD"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","08_WRIST"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","09_HANDS"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","10_WAIST"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","11_LEGS"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","12_FEET"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","13_OFFHAND"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","ARMOR"), 13, reset)
  self:SetCatBar(cfg, strformat("SOULBOUND_%s","WEAPON"), 13, reset)

  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","RING"), 14, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","TRINKET"), 14, reset)

  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","01_HEAD"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","02_NECK"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","03_SHOULDER"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","04_BACK"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","05_CHEST"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","06_SHIRT"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","07_TABARD"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","08_WRIST"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","09_HANDS"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","10_WAIST"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","11_LEGS"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","12_FEET"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","13_OFFHAND"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","ARMOR"), 13, reset)
  self:SetCatBar(cfg, strformat("ACCOUNTBOUND_%s","WEAPON"), 13, reset)
...
-- Third default line - Swappables
  ...
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","RING"), 10, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","TRINKET"), 10, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","OTHER"), 10, reset)
  self:SetCatBar(cfg, "LEGENDARIES", 10, reset)

  self:SetCatBar(cfg, strformat("EQUIPPED_%s","01_HEAD"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","02_NECK"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","03_SHOULDER"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","04_BACK"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","05_CHEST"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","06_SHIRT"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","07_TABARD"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","08_WRIST"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","09_HANDS"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","10_WAIST"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","11_LEGS"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","12_FEET"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","13_OFFHAND"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","ARMOR"), 9, reset)
  self:SetCatBar(cfg, strformat("EQUIPPED_%s","WEAPON"), 9, reset)
...
function TBag:InitDefVals(cfg, bagarr, row1offset, reset)
  ...
  for idx, bag in ipairs(bagarr) do
    ...
    self:SetColor(cfg, "bag_"..bag, self.DBC[idx]["r"], self.DBC[idx]["g"], self.DBC[idx]["b"], self.DBC[idx]["a"], reset)
  end
...
function TBag:AssignCats(cfg, reset)
  ...
  for _ ,value in ipairs(cfg["item_search_list"]) do
    if (self:GetCat(cfg, value[1]) == nil) then
      DEFAULT_CHAT_FRAME:AddMessage(self.SCP..strformat(L["Unassigned category %s has been assigned to slot 1"],value[1]))
...
function TBag:DeleteCachedCharacter(char,realm)
  ...
  if (found == 1 and TInvItm[playerid] == nil) then
    DEFAULT_CHAT_FRAME:AddMessage(self.SCP..strformat(L["Removed cache for %q"],playerid), 1, 1, 1)
  else
    DEFAULT_CHAT_FRAME:AddMessage(self.SCP..strformat(L["Couldn't find and remove cache for %q"],playerid), 1, 1, 1)
  end
end
...
function TBag:SetFrameAnchor (frame,cfg,y,x)
  ...
  frame:SetPoint(cfg["frameYRelativeTo"]..cfg["frameXRelativeTo"], "UIParent", "BOTTOMLEFT", cfg["frame"..cfg["frameXRelativeTo"]] / frame:GetScale(), cfg["frame"..cfg["frameYRelativeTo"]] / frame:GetScale())
end
...
function TBag:Cat(str)
  ...
  local cat = strupper(str)
  return strgsub(cat, " ", "_")
end
...
function TBag:GetBagType(playerid, bag)
  ...
  if (playerid == self.PLAYERID and (TBnkFrame.atbank == 1 or self:Member(TBag.Inv_Bags, bag))) then
    ...
    if (bag == REAGENTBANK_CONTAINER) then
      type = 2048 -- This is an unused id and we're squating on it since the reagent bank doesn't have a real id
    end
...
--    self:Print("b="..bag..", size="..size)
      for i=1, size do
        local containerInfo = GetContainerItemInfo(bag, i)
...
--    end
  ...
  for buttonname, itm in pairs(self.BUTTONS) do
    ...
    if (texture) and (itm) and next(itm) then
      ...
      if (self:GetBagFrame(bag):GetChecked() or isopen[bag]) and (cfg) and (cfg["spotlight_open"] == 1) then
        texture:Show()
      else
        texture:Hide()
      end
...
function TBag:MakeColorMenu(cfg, updatefunc, level, bagarr)
  ...
  info = self:MakeColorPickerInfo(cfg, "bkgr_", self.MAIN_BAR, L["Main Background Color"], updatefunc)
  TBag.LibDD:UIDropDownMenu_AddButton(info, level)

  info = self:MakeColorPickerInfo(cfg, "brdr_", self.MAIN_BAR, L["Main Border Color"], updatefunc)
  TBag.LibDD:UIDropDownMenu_AddButton(info, level)
  ...
  for _, bag in ipairs(bagarr) do
    info = self:MakeColorPickerInfo(cfg, "bag_", bag, strformat(L["Spotlight for %s"],self:GetBagDispName(bag)), updatefunc)
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)
  end
end
...
function TBag:UpdateHearth(tt, itemlink, playerid)
  ...
  -- as well.
  if (playerid ~= self.PLAYERID and (strmatch(itemlink,"item:6948:") or strmatch(itemlink,"item:28585:"))) then
    local hearth
    ...
    local repl = strformat(L["%%1%s%%3"],hearth)
    ...
    for i=1, tt:NumLines() do
      ...
      if (ttleft) then
        ...
        if (line) then
          local sub,match = strgsub(line, L["(Use: Returns you to )([^%.]*)(%.)"],repl,1)
          if (match == 1) then
            ttleft:SetText(sub)
            tt:Show()
            break
  ...
  if (playerid == self.PLAYERID) then
    ...
    if (self:Member(self.Inv_Bags, bag) or TBnkFrame.atbank == 1) then
      ...
      if itemlink and itemlink ~= "" then
        local level = TBag:GetPlayerInfo(playerid,TBag.G_BASIC)[TBag.S_LEVEL] or UnitLevel("player")
...
function TBag:InsertEmptyInCompArr(ca,itm)
  ...
  if (bagtype and bagtype > 0) then
    tinsert(ca[self.COMP_EMPTY], itm)
  end
end
...
function TBag:InsertStackArr(stackarr,itm,id)
  ...
  if (itm[self.I_NEED] > 0) then
    ...
    if (self:GetStackSkip(itm[self.I_BAG], itm[self.I_SLOT]) == nil) then
      self:PrintDEBUG("Stack inserting ("..itm[self.I_BAG]..", "..itm[self.I_SLOT]..") with need="..itm[self.I_NEED])
      stackarr[id] = stackarr[id] or {}
      tinsert(stackarr[id],itm)
    end
  end
end
...
function TBag:GetItmCharges(tooltip)
  return strmatch(tooltip, L["(%d+) Charges?"])
end
...
function TBag:UpdateItmCache(cfg, playerid, itmcache, bagarr, stackarr, comparr, atbank)
  ...
  local forBagSlot = {self.I_NAME, self.I_SUBTYPE_EL, self.I_COUNT, self.I_RARITY, self.I_SUBTYPE, self.I_TYPE, self.I_NEED,
                      ...
                      self.I_SOULBOUND, self.I_ACCTBOUND, self.I_CRAFTINGREAGENT, self.I_CHARGES, self.I_ITEMLVL}
...
--    if (cfg["show_Bag"..bag] == 1) then
    ...
    if (size > 0) then
      ...
      for slot = size, 1, -1 do
        ...
        itm[self.I_ITEMLVL] = itmcache[bag][slot][self.I_ITEMLVL]
        ...
        if (itm[self.I_ITEMLINK] ~= nil) then
          ...
          if itemType == 2 or itemType == 4 then
            ...
            if itemLvl and TBagCfg["Inv"]["show_lvl"] then
              itm[self.I_ITEMLVL] = tostring(itemLvl)
              if itemLvl > 183 then
                local _, bonusStat = strmatch(itemLink, "^|c%x+|Hitem:%d+:%d*:%d*:%d*:%d*:%d*:%d*:%d*:%d*:%d*:%d*:%d*:(%d*):(.*)|h%[.*%]")
                  if not strfind(bonusStat, "6652:") then
                    itm[self.I_ITEMLVL] = itm[self.I_ITEMLVL] .. "+"
                  end
                  if strfind(bonusStat, "6935:") then
                    itm[self.I_ITEMLVL] = itm[self.I_ITEMLVL] .. "!"
                  end
              end
            end
          ...
          if (itm[self.I_CHARGES]) then
            ...
            if self:GetItmCharges(tooltip) then
        ...
        if (itm[self.I_BAR] == nil and (cfg["show_Bag"..bag] == 1 or TBag:GetBagFrame(bag):GetChecked())) then
          resort_mandatory = 1
        end
        ...
        if (itm[self.I_ITEMLINK] ~= itmcache[bag][slot][self.I_ITEMLINK]) then
          ...
          if (strfind(tooltip, L["Soulbound"])) then
            itm[self.I_SOULBOUND] = 1
          else
            itm[self.I_SOULBOUND] = 0
          end
          if (strfind(tooltip, L["Account Bound"])) then
          ...
          if (strfind(tooltip, PROFESSIONS_USED_IN_COOKING)) then
...
--    self:PrintDEBUG("TBag:MakeBarItm: bag ="..bag)
    ...
    if (cfg["show_Bag"..bag] == 1 or TBag:GetBagFrame(bag):GetChecked()) then
      size = tgetn(itmcache[bag])
      if (size > 0) then
--        self:PrintDEBUG("Show bag "..bag)
        for slot = 1, size do
          if next(itmcache[bag][slot]) then
            itmcache[bag][slot] = self:PickBar(cfg, playerid, itmcache[bag][slot], trade1, trade2, atbank)
            tinsert( baritm[ itmcache[bag][slot][self.I_BAR] ], itmcache[bag][slot])
          end
        end
      end
    end
  ...
  for barnum = 1, self.BAR_MAX do
    ...
    if (toggle==1 or toggle==2) then
      tsort(baritm[barnum],
        function(a,b) return
          a[TBag.I_CAT]..TBag:ReverseString(a[TBag.I_NAME],toggle)..strformat("%04s",a[TBag.I_COUNT])..strformat("%02s",a[TBag.I_SLOT]) > b[TBag.I_CAT]..TBag:ReverseString(b[TBag.I_NAME],toggle)..strformat("%04s",b[TBag.I_COUNT])..strformat("%02s",b[TBag.I_SLOT])
        end
      )
    end
  end
...
function TBag:PickBar(cfg, playerid, itm, trade1, trade2, atbank)
  ...
  if (itm[self.I_ITEMLINK] == nil) then
    if (bagtype and type(bagtype) == "number" and bagtype > 0) then
      itm[self.I_CAT] = strformat(L["EMPTY_%s_SLOTS"],self:GetBagTypeName(bagtype))
    elseif (bagtype and type(bagtype) == "string" and bagtype ~= "") then
      ...
      itm[self.I_CAT] = strformat(L["EMPTY_%s_SLOTS"],bagtype)
    elseif (bagtype and type(bagtype) == "string" and bagtype ~= "") then

    else
      itm[self.I_CAT] = strformat(L["EMPTY_%s_SLOTS"], self:GetBagPosName(itm[self.I_BAG]))
    end
  ...
  if (bagtype and ((type(bagtype) == number and bagtype > 0) or
    ...
    if (cfg["special_bag_sort"] == 1) then
      if (type(bagtype) == "number") then
        itm[self.I_CAT] = strformat(L["IN_%s_BAG"],self:GetBagTypeName(bagtype))
      else
        -- Support for old style string bag types.
        itm[self.I_CAT] = strformat(L["IN_%s_BAG"],bagtype)
      end
...
--      self:SetCatBar(cfg, strformat(L["%s_CREATED"],self:Cat(trade1)),
--                     strformat(L["%s_CREATED"],L["TRADE1"]), 1)
--    else
--      self:SetCatBar(cfg, strformat(L["%s_CREATED"],self:Cat(trade1)), nil, 1)
--    end
--  end
--  if (trade2 ~= "") then
--    self:SetCatBar(cfg, self:Cat(trade2), L["TRADE2"], 1)
--    if (cfg["trade_created_sort"] == 1) then
--      self:SetCatBar(cfg, strformat(L["%s_CREATED"],self:Cat(trade2)),
--                     strformat(L["%s_CREATED"],L["TRADE2"]), 1)
--    else
--      self:SetCatBar(cfg, strformat(L["%s_CREATED"],self:Cat(trade2)), nil, 1)
--    end
--  end
  ...
  if (itm[self.I_CAT] == nil) then
    for i = 1, tgetn(cfg["item_search_list"]) do
      ...
      if (value[1] ~= "") then
        ...
        -- check tooltip
        if ( (value[3] ~= "") and (not (strfind(tooltip, value[3]))) ) then
          found = nil
        end
        -- check itemType
      ...
      for slot = 1, itemCount do
        ...
        if itemlink and itemlink:sub(1,11)  == "item:82800:" then
          ...
          if speciesID and speciesID > 0 then
            itemlink = strformat("battlepet:%d:%d:%d:%d:%d:%d:0",speciesID, level, breedQuality, maxHealth, power, speed)
            name = petname
        ...
        local tooltip = self:MakeToolTipStr(playerid, itm[self.I_ITEMLINK], nil, nil, idx, slot)
        itm[self.I_CHARGES] = self:GetItmCharges(tooltip)
      end
    end
  end
...
function TBag:CalcBarLayout(calc_dat, baritm, barnum, numbars, colmax, edit_mode)
  ...
  for k in pairs(calc_dat) do
    calc_dat[k] = nil
  end
  for iBar = 0, numbars-1 do
    if (edit_mode == 1) then
      calc_dat[iBar] = tgetn(baritm[barnum+iBar]) + 1
    else
      if baritm[barnum+iBar] then
        calc_dat[iBar] = tgetn(baritm[barnum+iBar])
      else
        calc_dat[iBar] = 0
      end
    end
  ...
  for iBar = 0, numbars-1 do
    ...
    if calc_dat[iBar] > 0 then
      for tmpcalc = 1, calc_dat[iBar] do
        calc_dat[iBar.."_heights"][tmpcalc] = mathceil( calc_dat[iBar] / tmpcalc )
      end
    end
  end
...
function TBag:AssignButtonsToFrame(mainFrame, barnum, frame, width, height)
  ...
  for position, itm in pairs(mainFrame.BARITM[barnum]) do
    ...
    self:PositionFrame(buttonname, "BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0-mainFrame:FrameX(cur_x)+mainFrame.BF_X_PAD, mainFrame:FrameY(cur_y)+mainFrame.BF_Y_PAD, mainFrame.BF_WIDTH, mainFrame.BF_HEIGHT)

    -- resize frame texture (this is the little border)
    self:PositionFrame(buttonname.."_bkgr", "TOPLEFT", buttonname, "TOPLEFT", 0-mainFrame.BF_X_PAD, mainFrame.BF_Y_PAD, mainFrame.BGF_WIDTH, mainFrame.BGF_HEIGHT)
  ...
  if mainFrame.edit_mode == 1 then
    ...
    self:PositionFrame(buttonname, "BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0-mainFrame:FrameX(width-1)+mainFrame.BF_X_PAD, mainFrame:FrameY(height-1)+mainFrame.BF_Y_PAD, mainFrame.BF_WIDTH, mainFrame.BF_HEIGHT)
...
function TBag:GetBarY(bar_x)
  return mathfloor(self.BAR_MAX / bar_x)
end
...
function TBag:LayoutWindow(frame)
  ...
  local available_width = frame:FrameX(cfg["maxColumns"]) + frame:SpaceX(bar_x-1) + frame:PoolX(bar_x+1) + (2 * self.BORDER)
  local width_in_between

  if (framename == "TInvFrame") then
    if (TInv_SearchBox:IsVisible() or TInvFrame_Total:IsVisible() or TInvacterBag3Slot:IsVisible() or TInvFrame_MoneyFrame:IsVisible()) then
    ...
    if ((TInv_SearchBox:IsVisible() and (TInvFrame_Total:IsVisible() or TInvacterBag3Slot:IsVisible())) or TInvFrame_MoneyFrame:IsVisible() and TInvFrame_TokenFrame:IsVisible()) then
      PAD_BOTTOM = PAD_BOTTOM + self.PAD_BOTTOM_SEARCH
    end
    local tokensWatched = TBag.Tokens.Watched
    local tokenPadRows = mathceil(tokensWatched / 3 - 2)
    ...
    if (TInv_UserDropdown:IsVisible() or TInv_Button_HighlightToggle:IsVisible() or TInv_Button_ChangeEditMode:IsVisible() or TInv_Button_ShowBank:IsVisible() or TInv_Button_Reload:IsVisible() or TInv_Button_Close:IsVisible() or TInv_Button_MoveLockToggle:IsVisible()) then
      PAD_TOP = self.PAD_TOP_NORM
    end
  else
    -- TBnkFrame
    if (TBnkFrame_Total:IsVisible() or TBnkFrameBagBank:IsVisible() or TBnkFrame_MoneyFrame:IsVisible() or TBnkFrame_TokenFrame:IsVisible()) then
      PAD_BOTTOM = PAD_BOTTOM + self.PAD_BOTTOM_NORM
    end
    if (edit_mode == 1) then
      PAD_BOTTOM = PAD_BOTTOM + self.PAD_BOTTOM_EDIT
    ...
    if (cfg["maxColumns"] <= bags_row or (TBnkFrame_MoneyFrame:IsVisible() and TBnkFrame_TokenFrame:IsVisible())) then
      PAD_BOTTOM = PAD_BOTTOM + self.PAD_BOTTOM_NORM
    end
    ...
    if (TBnk_UserDropdown:IsVisible() or TBnk_Button_HighlightToggle:IsVisible() or TBnk_Button_ChangeEditMode:IsVisible() or TBnk_Button_Reload:IsVisible() or TBnk_Button_DepositReagent:IsVisible() or TBnk_Button_Close:IsVisible() or TBnk_Button_MoveLockToggle:IsVisible()) then
      PAD_TOP = self.PAD_TOP_NORM
    end
  end
  ...
  for barnum = 1, bar_x * bar_y, bar_x do
    ...
    self:CalcBarLayout(calc_dat, baritm, barnum, bar_x, cfg["maxColumns"], edit_mode)
    ...
    if (calc_dat["height"] == 0) then
      ...
      width_in_between = frame:FrameX(cfg["maxColumns"]) + frame:SpaceX(bar_x-1) + frame:PoolX(bar_x-1)
      for iBar = 0, bar_x - 1 do
      ...
      for iBar = 0, bar_x - 1 do
        if (calc_dat[iBar.."_width"] >= 0 and (tgetn(baritm[barnum+iBar]) > 0 or edit_mode == 1)) then
          -- Keep width separate to get roundoff staggering
          if (bar_x == 1) then
            cur_width = 0
          else
            cur_width = mathfloor(iBar * width_in_between / (bar_x - 1))
          end

          self:PositionFrame(framename.."_bar_"..(barnum+iBar), "BOTTOMRIGHT", framename, "BOTTOMRIGHT", 0-cur_x-cur_width, cur_y, frame:FrameX(calc_dat[iBar.."_width"]), frame:FrameY(calc_dat["height"]))
          ...
          TBag:AssignButtonsToFrame(frame,(barnum+iBar), framename.."_bar_"..(barnum+iBar), calc_dat[iBar.."_width"], calc_dat["height"])
          barframe[iBar]:Show()
        else
          barframe[iBar]:Hide()
        end
...
function TBag:Stack(where, itmcache, sa, ca)
  ...
  for itemid,itms in pairs(sa) do
    ...
    tsort(itms, function(a,b)
      if (a[TBag.I_COUNT] == b[TBag.I_COUNT]) then
        -- We only ever stack when on the current player so this is ok.
        return (TBag:GetBagType(TBag.PLAYERID,a[TBag.I_BAG]) or 0) > (TBag:GetBagType(TBag.PLAYERID,b[TBag.I_BAG]) or 0)
      else
    ...
    if (src > 1) then
      ...
      while (src > dest) do
        ...
        if (destitm[self.I_NEED] >= srcitm[self.I_COUNT]) then
          ...
          self:ItemMover(srcitm[self.I_BAG], srcitm[self.I_SLOT], destitm[self.I_BAG], destitm[self.I_SLOT])
          ...
          self:ItemMover(srcitm[self.I_BAG], srcitm[self.I_SLOT], destitm[self.I_BAG], destitm[self.I_SLOT], destitm[self.I_NEED])
  ...
  if (ca and type(ca) == "table") then
    local epts = ca[self.COMP_EMPTY]
    local itms = ca[self.COMP_ITEM]
    local empty_size = tgetn(epts)
    local items_size = tgetn(itms)
...
function TBag:ItemMover(bag1, slot1, bag2, slot2, count)
  ...
  tinsert(ItemMover__instructions,1,inst)
end
...
local function ItemMover__main(instructions)
  ...
  while true do
    instruction_count = tgetn(instructions)
...
function TBag:FixMenuFrameLevels()
  for l=1,L_UIDROPDOWNMENU_MAXLEVELS do
    for b=1,L_UIDROPDOWNMENU_MAXBUTTONS do
      local button = _G["L_DropDownList"..l.."Button"..b]
      if button then
