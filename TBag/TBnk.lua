...
--    end
  ...
  TBag:CreateFrame("Frame", "TBnkFrame_bar_", TBnkFrame, "TBag_BarFrameTemplate", TBag.BAR_MAX, "")
  TBag:CreateFrame("Button", "TBnkFrame_BarButton_", TBnkFrame, "TBag_BarButtonTemplate", TBag.BAR_MAX, "")
  ...
  self.BARITM = TBag:SortItmCache(cfg, self.playerid, TBnkItm[self.playerid], self.BARITM, self.bags, self.atbank)
...
function Bank:UpdateBagGfx()
  ...
  for i=1, NUM_BANKBAGSLOTS do
    ...
    TBag:GetBagFrameTexture(bag):SetTexture(TBag:GetBagTexture(self.playerid, bag))
...
function Bank:InitBagGfx()
  ...
  TBag:GetBagFrameTexture(BANK_CONTAINER):SetTexture(TBag:GetBagTexture(TBnkFrame.playerid, BANK_CONTAINER))
  TBag:GetBagFrameTexture(REAGENTBANK_CONTAINER):SetTexture(TBag:GetBagTexture(TBnkFrame.playerid, REAGENTBANK_CONTAINER))
...
function Bank.SlotTargetButton_OnClick(self, button)
  ...
  if (TBnkFrame.edit_mode == 1) then
    for tmp in strgmatch(self:GetName(), "TBnkFrame_SlotTarget_(%d+)") do
      bar = tonumber(tmp)
    end
...
function Bank.Toggle_BagSlotButtons()
  local BagButtons = { "1", "2", "3", "4", "5", "6", "7", "Bank", "Reagent" }
  if (TBnkFrame.cfg["show_bagbuttons"] == 1) then
    TBnkFrame.cfg["show_bagbuttons"] = 0
    for _, i in pairs(BagButtons) do
      _G['TBnkFrameBag'..i]:Hide()
    end
    TBnkFrame:SetButton_Anchors()
  else
    TBnkFrame.cfg["show_bagbuttons"] = 1
    for _, i in pairs(BagButtons) do
      _G['TBnkFrameBag'..i]:Show()
    end
    TBnkFrame:SetButton_Anchors()
   end
end
...
function Bank.RightClickMenu_populate(self, level)
  ...
  if (TBnkFrame.RightClickMenu_mode == "item") then
    ...
    if (level == 1) then
      ...
      info = { ["text"] = strformat(L["Current Category: %s"],itm[TBag.I_CAT]), ["notClickable"] = 1, ["isTitle"] = 1, ["notCheckable"] = nil }
      TBag.LibDD:UIDropDownMenu_AddButton(info, level)
    ...
    elseif (level == 2) then
      if ( L_UIDROPDOWNMENU_MENU_VALUE == "override_placement" ) then
        for i = 1, TBag.BAR_MAX do
          info = {
            ["text"] = strformat(L["Categories within bar %d"],i),
            ["value"] = { ["opt"]="override_placement_select", [TBag.I_BAG]=bag, [TBag.I_SLOT]=slot, ["select_bar"]=i },
            ["hasArrow"] = 1
          }
          if ( (TBnkFrame.cfg["item_overrides"][id] ~= nil) and (TBag:GetCat(TBnkFrame.cfg, TBnkFrame.cfg["item_overrides"][id]) == i) ) then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
        end
  ...
  elseif (TBnkFrame.RightClickMenu_mode == "bar") then
    ...
    info = { ["text"] = strformat(L["|c%sBar |r|c%s%s|r"],TBag.C_INST,TBag.C_BAR,bar), ["notClickable"] = 1, ["isTitle"] = 1, ["notCheckable"] = nil }
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)
    ...
    for key, value in pairs(TBnkFrame.BC_LIST[bar]) do
      info = {
        ["text"] = strformat(L["Move: |c%s%s|r"],TBag.C_CAT,value),
        ["value"] = value,
        ["func"] = function(self)
          local this = self or _G.this
    ...
    info = TBag:MakeColorPickerInfo(TBnkFrame.cfg, "bkgr_", bar, strformat(L["Background Color for Bar %d"],bar), function () TBnkFrame:UpdateWindow() end)
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)

    info = TBag:MakeColorPickerInfo(TBnkFrame.cfg, "brdr_", bar, strformat(L["Border Color for Bar %d"],bar), function () TBnkFrame:UpdateWindow() end)
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)
  ...
  elseif (TBnkFrame.RightClickMenu_mode == "mainwindow") then
    if (level == 1) then
      ...
      info = { ["text"] = strformat(L["TBag v%s"],TBag.VERSION), ["notClickable"] = 1, ["isTitle"] = 1, ["notCheckable"] = nil }
      TBag.LibDD:UIDropDownMenu_AddButton(info, level)
      ...
      if (L_UIDROPDOWNMENU_MENU_VALUE ~= nil) then
        if (L_UIDROPDOWNMENU_MENU_VALUE["opt"] == "set_scale") then
          for _, value in ipairs(TBag.A_BUTTONSIZE) do
            ...
            if (tonumber(TBnkFrame.cfg["frameButtonSize"]*TBnkFrame.cfg["scale"] - value) < 1.0) and (tonumber(TBnkFrame.cfg["frameButtonSize"]*TBnkFrame.cfg["scale"] - value) > -1.0) then
              info["checked"] = 1
            end
            TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          end
        ...
        elseif (L_UIDROPDOWNMENU_MENU_VALUE["opt"] == "anchor") then
          ...
          if (TBnkFrame.cfg["frameXRelativeTo"] == "LEFT" and TBnkFrame.cfg["frameYRelativeTo"] == "TOP") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TBnkFrame.cfg["frameXRelativeTo"] == "RIGHT" and TBnkFrame.cfg["frameYRelativeTo"] == "TOP") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TBnkFrame.cfg["frameXRelativeTo"] == "LEFT" and TBnkFrame.cfg["frameYRelativeTo"] == "BOTTOM") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TBnkFrame.cfg["frameXRelativeTo"] == "RIGHT" and TBnkFrame.cfg["frameYRelativeTo"] == "BOTTOM") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
...
function Bank:UpdateWindow(resort_req)
  ...
  if (resort_req >= TBag.REQ_MUST) then
    self.BARITM = TBag:SortItmCache(self.cfg, self.playerid, TBnkItm[self.playerid], self.BARITM, self.bags, self.atbank)
    TBag:LayoutWindow(self)
  ...
  frame:SetPoint(self.cfg["frameYRelativeTo"]..self.cfg["frameXRelativeTo"], "UIParent", "BOTTOMLEFT", self.cfg["frame"..self.cfg["frameXRelativeTo"]] / frame:GetScale(), self.cfg["frame"..self.cfg["frameYRelativeTo"]] / frame:GetScale())
...
function Bank.UserDropdown_Initialize(self, level)
  TBag:UserDropdown_Init(Bank.UserDropdown_OnClick, TBnkItm, TBnkFrame.playerid, TBag.REALM, level)
end
