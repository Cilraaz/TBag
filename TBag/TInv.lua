...
function Inv:init(reset)
  ...
  TBag:CreateFrame("Frame", "TInvFrame_bar_", TInvFrame, "TBag_BarFrameTemplate", TBag.BAR_MAX, "")
  TBag:CreateFrame("Button", "TInvFrame_BarButton_", TInvFrame, "TBag_BarButtonTemplate", TBag.BAR_MAX, "")
...
function Inv.RightClickMenu_populate(self, level)
    ...
    elseif (level == 2) then
      if ( L_UIDROPDOWNMENU_MENU_VALUE == "override_placement" ) then
        for i = 1, TBag.BAR_MAX do
          ...
          if ((TInvFrame.cfg["item_overrides"][id] ~= nil) and (TBag:GetCat(TInvFrame.cfg, TInvFrame.cfg["item_overrides"][id]) == i)) then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
        end
  ...
  elseif (TInvFrame.RightClickMenu_mode == "bar") then
    ...
    info = TBag:MakeColorPickerInfo(TInvFrame.cfg, "bkgr_", bar, strformat(L["Background Color for Bar %d"],bar), function() TInvFrame:UpdateWindow() end)
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)

    info = TBag:MakeColorPickerInfo(TInvFrame.cfg, "brdr_", bar, strformat(L["Border Color for Bar %d"],bar), function() TInvFrame:UpdateWindow() end)
    TBag.LibDD:UIDropDownMenu_AddButton(info, level)
        ...
        ["func"] = function()
          ...
          for index, bag in ipairs(TInvFrame.bags) do
            if (TInvFrame.cfg["show_Bag"..bag] == 1) then
              if (tgetn(TInvItm[TInvFrame.playerid][bag]) > 0) then
                for slot = 1, tgetn(TInvItm[TInvFrame.playerid][bag]) do
      ...
      if (L_UIDROPDOWNMENU_MENU_VALUE ~= nil) then
        if (L_UIDROPDOWNMENU_MENU_VALUE["opt"] == "set_scale") then
          for _, value in ipairs(TBag.A_BUTTONSIZE) do
            ...
            if (tonumber(TInvFrame.cfg["frameButtonSize"]*TInvFrame.cfg["scale"] - value) < 1.0) and (tonumber(TInvFrame.cfg["frameButtonSize"]*TInvFrame.cfg["scale"] - value) > -1.0) then
              info["checked"] = 1
            end
            TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          end
        ...
        elseif (L_UIDROPDOWNMENU_MENU_VALUE["opt"] == "anchor") then
          ...
          if (TInvFrame.cfg["frameXRelativeTo"] == "LEFT" and TInvFrame.cfg["frameYRelativeTo"] == "TOP") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TInvFrame.cfg["frameXRelativeTo"] == "RIGHT" and TInvFrame.cfg["frameYRelativeTo"] == "TOP") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TInvFrame.cfg["frameXRelativeTo"] == "LEFT" and TInvFrame.cfg["frameYRelativeTo"] == "BOTTOM") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
          info = {
          ...
          if (TInvFrame.cfg["frameXRelativeTo"] == "RIGHT" and TInvFrame.cfg["frameYRelativeTo"] == "BOTTOM") then
            info["checked"] = 1
          end
          TBag.LibDD:UIDropDownMenu_AddButton(info, level)
...
function Inv:UpdateWindow(resort_req)
  ...
  TBag:PrintDEBUG("TInv_UpdateWindow: resort_req is "..tostring(resort_req))
  ...
  if (resort_req >= TBag.REQ_MUST) then
    self.CACHE_REQ = TBag.REQ_NONE
    self.BARITM = TBag:SortItmCache(self.cfg, self.playerid, TInvItm[self.playerid], self.BARITM, self.bags)
    TBag:LayoutWindow(self)
...
function Inv.UserDropdown_Initialize(self, level)
  TBag:UserDropdown_Init(Inv.UserDropdown_OnClick, TInvItm, TInvFrame.playerid,TBag.REALM,level)
end
